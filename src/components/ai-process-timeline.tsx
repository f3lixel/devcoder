...
export type ProcessStepState = "pending" | "done" | "error";
export type ProcessStepType = "create" | "edit" | "delete" | "analyze" | "generate";
...
export function ProcessTimeline({ steps, phase, progress, onClose, autoCollapseMs = 2000 }: ProcessTimelineProps) {
  const [collapsed, setCollapsed] = React.useState(false);
  const statusText = phase === "running" ? "Verarbeitung läuft" : phase === "complete" ? "Verarbeitung abgeschlossen" : "Verarbeitung fehlgeschlagen";
  const statusColor = phase === "running" ? "text-amber-400" : phase === "complete" ? "text-emerald-400" : "text-red-400";

  const StepIcon: React.FC<{ type: ProcessStepType; state: ProcessStepState }> = ({ type, state }) => {
    ...
    if (type === "delete") return <Trash2 className={base} />;
    if (type === "analyze") return <Loader2 className={`${base} animate-spin`} />;
